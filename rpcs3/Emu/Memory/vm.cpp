...
namespace vm
{
	...
	std::array<atomic_t<u64>, 6> g_cond_locks{};
	...
	static atomic_t<u64>* _register_cond_lock(const u64 lock_info)
	{
		while (true)
		{
			for (auto& lock : g_cond_locks)
			{
	...
	atomic_t<u64>* passive_lock(const u32 addr, const u32 size)
	{
		static const auto test_addr = [](const u32 target, const u32 addr, const u32 size)
		{
		...
		if (LIKELY(test_addr(g_addr_lock.load(), addr, size)))
		{
			// Optimistic path (hope that address range is not locked)
			_ret = _register_cond_lock((u64)size << 32 | addr);
		...
		if (addr)
		{
			...
			for (auto& lock : g_cond_locks)
			{
				while (true)
				{
					...
					const u32 size = value >> 32;

					// Test end address
					if (static_cast<u32>(value) + size <= addr)
					{
						break;
					}
